...
class DABDetrTransformerDecoder(DetrTransformerDecoder):
    ...
    def forward(self,
        ...
        reference_points = unsigmoid_references.sigmoid()  # [2, 300, 4]
        ...
        for layer_id, layer in enumerate(self.layers):
            obj_center = reference_points[..., :self.query_dim]   # [2, 300, 4]
            ref_sine_embed = coordinate_to_encoding(
                coord_tensor=obj_center, num_feats=self.embed_dims // 2)  # [2, 300, 512]
            ...
            if self.query_scale_type != 'fix_elewise':
                if layer_id == 0:
                    # layer_id = 0 -> no transformation
                    pos_transformation = 1
                else:
                    pos_transformation = self.query_scale(output) # 根据上一层输出refine
            else:
                pos_transformation = self.query_scale.weight[layer_id]
            # apply transformation 0 / 1,2,3,4,5
