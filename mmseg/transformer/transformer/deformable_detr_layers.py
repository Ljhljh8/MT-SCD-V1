...
# TODO: change Deformable attention and FFN to norm type
# from mmcv.cnn.bricks.transformer import MultiheadAttention, FFN
from .mmcv_spike.transformer import MSDA_FFN, MultiheadAttention, FFN
from mmcv.ops import MultiScaleDeformableAttention
from .mmcv_spike import SpikeMultiScaleDeformableAttention
...
class SpikeDeformableDetrTransformerEncoder(DetrTransformerEncoder):
...
class DeformableDetrTransformerEncoder(DetrTransformerEncoder):
    ...
    def _init_layers(self) -> None:
        """Initialize encoder layers."""
        self.layers = ModuleList([
            SpikeDeformableDetrTransformerEncoderLayer(**self.layer_cfg)
            for _ in range(self.num_layers)
        ])
        self.embed_dims = self.layers[0].embed_dims
...
# NOTE: GO This branch
class SpikeDeformableDetrTransformerEncoderLayer(SpikeDetrTransformerEncoderLayer):
    """Encoder layer of Deformable DETR."""

    def _init_layers(self) -> None:
        """Initialize self_attn, ffn, and norms."""
        self.self_attn = SpikeMultiScaleDeformableAttention(**self.self_attn_cfg)
        self.embed_dims = self.self_attn.embed_dims
    ...
    def forward(self, query: Tensor, query_pos: Tensor,
        ...
        """
        query = query + self.gamma1 * self.self_attn(
            query=query,
            key=query,
            value=query,
            query_pos=query_pos,
        ...
        query = query + self.gamma2 * self.ffn(query)
