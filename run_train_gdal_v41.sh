...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=29710 train_WUSU_main_clean_pairbcd_V4.py \
  ...
  --relation-mode pdca \
  --use-pdca-guided-pair-decoder \
  --output_dir ./logs/v4_v4_freq_only \
  --pretrain_from '' \
  --val-mode all_pairs \
  --dend-mechanism-config v4_freq_only \
...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=29710 train_WUSU_main_clean_pairbcd_V4.py \
  ...
  --relation-mode pdca \
  --use-pdca-guided-pair-decoder \
  --output_dir ./logs/v4_v4_relation_only \
  --pretrain_from '' \
  --val-mode all_pairs \
  --dend-mechanism-config v4_relation_only \
...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=29710 train_WUSU_main_clean_pairbcd_V4.py \
  ...
  --relation-mode pdca \
  --use-pdca-guided-pair-decoder \
  --output_dir ./logs/v4_v4_full \
  --pretrain_from '' \
  --val-mode all_pairs \
  --dend-mechanism-config v4_full \
