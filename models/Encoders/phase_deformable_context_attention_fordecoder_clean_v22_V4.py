...
One class supports two construction-time-exclusive mechanisms:

* legacy V1--V3 K-based dendritic priors; or
* V4 bounded directed relation-logit priors.
...
class PhaseDeformableContextAttention(nn.Module):
    """Phase-Deformable Context Attention with exclusive prior interfaces.
    ...
    Forward:
        feat:   [N, B, C, H, W]
        K_GATE: legacy dendritic K maps from the current scale.
        relation_logit_prior: V4 target->source mapping of [B,1,H,W].
    ...
    def __init__(
        ...
        if (
            not self.legacy_dendritic_prior_enabled
            and self.pdca_dend_prior_mode != "none"
        ):
            raise ValueError(
                "legacy_dendritic_prior_enabled=False requires "
        ...
        if self.legacy_dendritic_prior_enabled:
            prior_alpha = (
                float(alpha)
                if pdca_dend_prior_alpha is None
                else float(pdca_dend_prior_alpha)
            )
            self.alpha = nn.Parameter(torch.tensor(prior_alpha))
        else:
            # A None-registered parameter is omitted from state_dict,
            # named_parameters, optimizer groups, and device conversions.
    ...
    @staticmethod
    def _format_relation_edges(edges) -> str:
        return "[" + ", ".join(
            "%s<-%s" % (target, source)
            for target, source in sorted(edges)
        ) + "]"

    def _expected_relation_edges(self):
        return {
            (target_name, source_name)
            for target_name in self.phase_names
            for source_name in self.source_names_by_target[target_name]
            if source_name != "__null__"
        }

    def _validate_relation_logit_prior(
        self,
        relation_logit_prior,
        batch: int,
        height: int,
        width: int,
        device: torch.device,
    ):
        if not isinstance(relation_logit_prior, Mapping):
            raise TypeError(
                "relation_logit_prior must be a target->source nested Mapping"
            )

        expected_edges = self._expected_relation_edges()
        expected_targets = {target for target, _source in expected_edges}
        provided_edges = set()
        values_by_edge = {}

        for target_name, source_mapping in relation_logit_prior.items():
            if not isinstance(target_name, str):
                raise ValueError("relation prior target keys must be phase-name strings")
            if target_name == "__null__" or target_name not in self.phase_to_index:
                raise ValueError("unknown relation prior target %r" % target_name)
            if not isinstance(source_mapping, Mapping):
                raise TypeError(
                    "relation_logit_prior[%r] must be a source Mapping"
                    % target_name
                )
            if target_name not in expected_targets and len(source_mapping) == 0:
                raise ValueError(
                    "relation prior target %r has no configured real sources"
                    % target_name
                )

            for source_name, prior in source_mapping.items():
                if not isinstance(source_name, str):
                    raise ValueError(
                        "relation prior source keys must be phase-name strings"
                    )
                if source_name == "__null__":
                    raise ValueError("__null__ must not appear in relation_logit_prior")
                if source_name not in self.phase_to_index:
                    raise ValueError("unknown relation prior source %r" % source_name)
                if source_name == target_name:
                    raise ValueError(
                        "self-edge is forbidden in relation_logit_prior: %s<-%s"
                        % (target_name, source_name)
                    )
                edge = (target_name, source_name)
                provided_edges.add(edge)
                values_by_edge[edge] = prior

        missing_edges = expected_edges - provided_edges
        extra_edges = provided_edges - expected_edges
        if missing_edges or extra_edges:
            raise ValueError(
                "relation_logit_prior graph mismatch: missing=%s, extra=%s"
                % (
                    self._format_relation_edges(missing_edges),
                    self._format_relation_edges(extra_edges),
                )
            )

        expected_shape = (batch, 1, height, width)
        for edge in sorted(expected_edges):
            prior = values_by_edge[edge]
            edge_name = "%s<-%s" % edge
            if not isinstance(prior, torch.Tensor):
                raise TypeError(
                    "relation prior %s must be a torch.Tensor" % edge_name
                )
            if tuple(prior.shape) != expected_shape:
                raise ValueError(
                    "relation prior %s shape must be %r, got %r"
                    % (edge_name, expected_shape, tuple(prior.shape))
                )
            if prior.device != device:
                raise ValueError(
                    "relation prior %s device %s does not match feature device %s"
                    % (edge_name, prior.device, device)
                )
            if not torch.is_floating_point(prior):
                raise TypeError("relation prior %s must have floating dtype" % edge_name)
            if not torch.isfinite(prior).all():
                raise FloatingPointError(
                    "relation prior %s contains NaN or Inf" % edge_name
                )
        return relation_logit_prior
    ...
    def forward(
        ...
        # Reject illegal interface combinations before the stateful activation
        # can mutate any membrane state.
        validated_relation_prior = None
        if self.legacy_dendritic_prior_enabled:
            if relation_logit_prior is not None:
                raise ValueError(
                    "legacy dendritic prior and relation_logit_prior are mutually exclusive"
                )
        else:
            if K_GATE is not None:
                raise ValueError(
                    "K_GATE must be None when legacy_dendritic_prior_enabled=False"
                )
            if relation_logit_prior is not None:
                validated_relation_prior = self._validate_relation_logit_prior(
                    relation_logit_prior,
                    batch=B,
                    height=H,
                    width=W,
        ...
        if self.legacy_dendritic_prior_enabled:
            ...
            dendritic_descriptor = None

        for target_idx, target_name in enumerate(self.phase_names):
            target = feat[target_idx]
            logits_by_source = []
            base_logits_by_source = [] if self.diagnostic_sink is not None else None
            diagnostic_final_logits_by_source = (
            ...
            for src_name in source_names:
                if src_name == "__null__":
                    ...
                    if base_logits_by_source is not None:
                        base_logits_by_source.append(null_logits)
                        diagnostic_final_logits_by_source.append(null_logits.float())
                ...
                base_logits = self.attn_head(evidence).view(
                    B,
                    self.num_heads,
                    self.num_points,
                    H,
                ...
                logits = base_logits
                diagnostic_final_logits = (
                    base_logits.float()
                    if diagnostic_final_logits_by_source is not None
                    else None
                )
                dend_prior_stats = None
                if self.legacy_dendritic_prior_enabled:
                ...
                elif validated_relation_prior is not None:
                    prior = validated_relation_prior[target_name][src_name]
                    # Device equality was validated.  Only dtype is converted,
                    # and non-in-place addition preserves base logits for A+.
                    prior_consumed = prior.to(dtype=base_logits.dtype)
                    logits = base_logits + prior_consumed[:, None, :, :, :]
                    if diagnostic_final_logits_by_source is not None:
                        diagnostic_final_logits = (
                            base_logits.float()
                            + prior_consumed.float()[:, None, :, :, :]
                        )
                    if self.diagnostic_sink is not None:
                        self.diagnostic_sink.record_relation_consumption(
                            block_index=self.diagnostic_block_index,
                            scale_index=self.diagnostic_scale_index,
                ...
                if base_logits_by_source is not None:
                    base_logits_by_source.append(base_logits)
                    diagnostic_final_logits_by_source.append(
                        diagnostic_final_logits
                    )
            ...
            if base_logits_by_source is not None:
                base_logits_stacked = torch.stack(base_logits_by_source, dim=2)
                diagnostic_base = base_logits_stacked.float()
                diagnostic_final = torch.stack(
                    diagnostic_final_logits_by_source,
                    dim=2,
                ).float()
                base_joint_weights = torch.softmax(
                    diagnostic_base.view(
                        B,
                        self.num_heads,
                        Q * self.num_points,
                        H,
                ...
                final_joint_weights = torch.softmax(
                    diagnostic_final.view(
                        B,
                        self.num_heads,
                        Q * self.num_points,
                        H,
                ...
                conditional_base = torch.softmax(diagnostic_base, dim=3)
                conditional_final = torch.softmax(diagnostic_final, dim=3)
                self.diagnostic_sink.record_relation_effect(
                    block_index=self.diagnostic_block_index,
                    scale_index=self.diagnostic_scale_index,
                    target_name=target_name,
                    source_names=source_names,
                    source_marginal_base=base_joint_weights.sum(dim=3),
                    source_marginal_final=final_joint_weights.sum(dim=3),
                    conditional_point_max_abs_error=(
                        conditional_base - conditional_final
                    ).abs().max(),
                )
