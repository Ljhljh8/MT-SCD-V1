...
from models.Encoders.phase_deformable_context_attention_fordecoder_clean_v22_evidence_unit import PhaseDeformableContextAttention
...
from models.mtscd_task_evidence_unit import MTSCDTaskEvidenceUnit
...
class FDPCEncoder(nn.Module):
    ...
    def __init__(
        ...
        task_evidence_cfg: Optional[dict] = None,
        task_evidence_scales: Iterable[int] = (3,),
        ...
        self.task_evidence_scales = set(int(s) for s in task_evidence_scales)
        self.task_evidence_enabled = bool(self.task_evidence_cfg.get("enabled", False))
        
        for s in self.task_evidence_scales:
            if s < 0 or s >= self.num_scales:
                raise ValueError(f"task evidence scale {s} is out of range")
        
        self.task_evidence_units = nn.ModuleDict()
        if self.task_evidence_enabled:
            unit_cfg = dict(self.task_evidence_cfg)
            unit_cfg.pop("enabled", None)
        
            for s in sorted(self.task_evidence_scales):
                self.task_evidence_units[str(s)] = MTSCDTaskEvidenceUnit(
                    channels=self.in_channels[s],
                    phase_names=self.phase_names,
                    pair_names=self.context_pairs,
                    enabled=True,
                    **unit_cfg,
                )
    ...
    def forward(
        ...
        for scale_key, unit in self.task_evidence_units.items():
            s = int(scale_key)
            encoded[s], evidence_aux = unit(
                encoded[s],
                return_aux=bool(return_aux),
                detach_aux=False,  # ���� encoder ��ǿ�� detach���� decoder gate ��������
            )
            if return_aux:
                aux["task_evidence"][scale_key] = evidence_aux
