...
@MODELS.register_module(name='AdaDilatedConv')
    ...
    def mix_forward(self, x):
        ...
        if hasattr(self, 'OMNI_ATT1') and hasattr(self, 'OMNI_ATT2'):
            ...
            normal_conv_x = F.conv2d(normal_conv_x.reshape(1, -1, h, w), adaptive_weight[:, -self.normal_conv_dim:].reshape(-1, self.in_channels // self.groups, self.kernel_size[0], self.kernel_size[1]), 
                                     bias=bias, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.normal_conv_dim * b)
...
if __name__ == '__main__':
    ...
    x = torch.rand(256, 384, 32, 32).cuda()
    # m = AdaptiveDilatedDWConv(in_channels=384, out_channels=384, kernel_size=3,padding=1,bias=False,groups=1).cuda()
    m = AdaptiveDilatedConv(in_channels=384, out_channels=384, kernel_size=3, padding=1, bias=False, groups=1).cuda()
    # m = nn.Conv2d(in_channels=384, out_channels=384, kernel_size=3, padding=1, bias=False, groups=1).cuda()
    m.eval()
    y = m(x)
    end_wall = time.time()
    print(y.shape)
    print(end_wall - start_wall)
