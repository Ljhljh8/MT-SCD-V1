...
class GSTMSCD_WUSU(nn.Module):
    def __init__(
        ...
        if self.dend_spatial_conv_type == "structure_routed_v4":
            if self.legacy_dendritic_prior_enabled:
                raise ValueError(
                    "structure_routed_v4 requires legacy_dendritic_prior_enabled=False"
                )
            if pdca_dend_prior_mode != "none":
                raise ValueError(
                    "structure_routed_v4 requires pdca_dend_prior_mode='none'"
                )
        else:
            if self.v4_frequency_enabled or self.v4_relation_enabled:
                raise ValueError(
                    "V4 Heads require dend_spatial_conv_type='structure_routed_v4'"
                )
            if not self.legacy_dendritic_prior_enabled:
                raise ValueError(
                    "legacy spatial operators require legacy_dendritic_prior_enabled=True"
                )
        ...
        self.encoder = nn.ModuleList(
            [
                FDPCEncoder(
                    ...
                    v4_relation_enabled=self.v4_relation_enabled,
                    legacy_dendritic_prior_enabled=(
    ...
    def set_v4_diagnostic_sink(self, sink) -> None:
        """Attach/detach a non-module A+ diagnostic accumulator."""
        for block in self.encoder:
            block.set_diagnostic_sink(sink)
