...
from models.Decoders.dysample import DySample
...
#         return x, k
class UpFuseBlock(nn.Module):
    """Dynamic-sampling upsample x to skip resolution and fuse.

    V1 only replaces the feature upsampling operator:
        old: F.interpolate(..., mode="bilinear")
        new: DySample(style="pl", dyscope=False)

    Input:
        x:    [N, B, Cx, H_low, W_low]
        skip: [N, B, Cs, H_high, W_high]

    Output:
        out:  [N, B, Cout, H_high, W_high]

    Notes:
        - N can be physical phase count or 1 for pair-batch path.
        - DySample sees [N*B, Cx, H_low, W_low].
        - This block does not mix phases or pairs.
    """
    ...
    def __init__(
        ...
        skip_channels: int,
        out_channels: int,
        use_DendSize: int = 128,
        groups: int = 1,
        # DySample V1 defaults.
        up_scale: int = 2,
        dysample_style: str = "pl",
        ...
        self.fuse1 = DendBlock(
            in_channels + skip_channels,
            out_channels,
            groups=out_channels,
        )
        self.fuse2 = ConvBlock(
            in_channels + skip_channels,
            out_channels,
            groups=out_channels,
        )
        ...
        self.dysample_style = str(dysample_style)
        self.dysample_groups = int(dysample_groups)
        self.dysample_scope = bool(dysample_scope)

        if self.dysample_style not in ("lp", "pl"):
            raise ValueError("dysample_style must be 'lp' or 'pl'.")

        if self.dysample_style == "pl":
            if in_channels < self.up_scale ** 2 or in_channels % (self.up_scale ** 2) != 0:
                raise ValueError(
                    "DySample style='pl' requires in_channels >= scale^2 and "
                    "in_channels % scale^2 == 0; got "
                    f"in_channels={in_channels}, scale={self.up_scale}."
                )

        if in_channels < self.dysample_groups or in_channels % self.dysample_groups != 0:
            raise ValueError(
                "DySample requires in_channels >= groups and in_channels % groups == 0; "
                f"got in_channels={in_channels}, groups={self.dysample_groups}."
            )

        self.dynamic_up = DySample(
            in_channels=in_channels,
            scale=self.up_scale,
            style=self.dysample_style,
            groups=self.dysample_groups,
            dyscope=self.dysample_scope,
        )
    ...
    def forward(self, x: torch.Tensor, skip: torch.Tensor, k=None) -> Tuple[torch.Tensor, Any]:
        # x: [N,B,Cx,H_low,W_low], skip: [N,B,Cs,H_high,W_high]
        N, B, Cx, _, _ = x.shape
        Hs, Ws = skip.shape[-2:]

        x_flat = x.flatten(0, 1).contiguous()          # [N*B,Cx,H_low,W_low]
        x_up = self.dynamic_up(x_flat)                 # [N*B,Cx,2H_low,2W_low]

        # Normal path should already match because current decoder scales are x2.
        # This fallback only protects odd-size inputs or future backbones.
        if x_up.shape[-2:] != (Hs, Ws):
            x_up = F.interpolate(x_up, size=(Hs, Ws), mode="bilinear", align_corners=False)

        x = x_up.reshape(N, B, Cx, Hs, Ws).contiguous()
...
class PDCAGuidedPairwiseChangeDecoder(nn.Module):
    ...
    def forward(
        ...
        for s, (feat, diff_proj) in enumerate(zip(feature_xy_high_to_low, self.diff_proj)):
            ...
            gates_by_pair: List[torch.Tensor] = []
            has_by_pair: List[bool] = []
            scale_key = str(s)
...
class MTSCDDecoderNet(nn.Module):
    ...
    def forward(
        ...
        if return_intermediates:
            outputs["sem_logits_seq"] = sem_logits_seq
            outputs["semantic_decoded_seq"] = sem_decoded
            if self.use_pdca_guided_pair_decoder:
                outputs["change_features"] = {"pair_gate_debug": pair_debug}
            else:
