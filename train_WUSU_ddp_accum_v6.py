...
def build_parser():
    ...
    parser.add_argument("--reference_batch_size", "--reference-batch-size", dest="reference_batch_size", type=int, default=2)
...
def validate_args(args):
    ...
    if args.reference_batch_size < 1 or args.reference_accum_steps < 1:
        raise ValueError("reference batch and accumulation must be >= 1")
...
def train_one_epoch(ctx, epoch):
    ...
    totals = {"loss": 0.0, "seg": 0.0, "bn": 0.0, "similarity": 0.0, "tl": 0.0}
    last_grad_norm = 0.0
    ...
    for step, (img1, img2, img3, mask1, mask2, mask3, mask_bn, sample_id) in enumerate(iterator):
        ...
        with sync_context:
            ...
            with torch.cuda.amp.autocast(enabled=False):
                loss, loss_seg, loss_bn, loss_similarity, loss_tl = compute_losses(
                    outputs,
...
def load_checkpoint(ctx, checkpoint_path):
    ...
    incompatible = unwrap_model(ctx.model).load_state_dict(strip_module_prefix(extract_model_state(checkpoint)), strict=False)
    if isinstance(checkpoint, dict):
...
def print_startup(args, train_micro_batches, updates_per_epoch, actual_updates, total_updates, warmup_updates):
    ...
    print(
        "reference_batch_size=%d, reference_accum_steps=%d, reference_effective_batch=%d"
        % (args.reference_batch_size, args.reference_accum_steps, reference_effective)
    )
    print(
    ...
    if effective_batch > reference_effective:
        print(
            "WARNING: effective batch is larger than reference batch; optimizer trajectory may differ from single-GPU baseline."
        )
