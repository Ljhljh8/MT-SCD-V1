...
def build_parser():
    ...
    parser.add_argument("--pdca-dend-prior-mode", default="offset_dual", choices=["none", "source", "source_gain", "offset_sim", "offset_dual"])
    ...
    parser.add_argument("--use-pdca-guidance", type=str2bool, default=True)
    parser.add_argument("--detach-pdca-guidance", type=str2bool, default=True)
    parser.add_argument("--collect-pdca-aux", type=str2bool, default=None)
    parser.add_argument("--task-evidence-enabled", type=str2bool, default=True)
    parser.add_argument("--task-evidence-feature-residual", type=str2bool, default=True)
    parser.add_argument("--task-evidence-use-highpass", type=str2bool, default=False)
    parser.add_argument("--task-evidence-detach", type=str2bool, default=True)
    parser.add_argument("--task-evidence-normalize", type=str2bool, default=True)
    parser.add_argument("--task-evidence-use-integer-surrogate", type=str2bool, default=False)
    parser.add_argument("--task-evidence-residual-init", type=float, default=0.001)
    parser.add_argument("--task-evidence-scale", type=int, default=3)
    parser.add_argument("--decoder-task-gate-enabled", type=str2bool, default=False)
    parser.add_argument("--decoder-task-gate-use-pdca-source-weights", type=str2bool, default=False)
    parser.add_argument("--decoder-task-gate-detach-evidence", type=str2bool, default=True)
    parser.add_argument("--decoder-task-gate-detach-pdca-guidance", type=str2bool, default=True)
    parser.add_argument("--decoder-task-gate-pdca-scale-key", default=None)
    parser.add_argument("--decoder-task-gate-alpha-max", type=float, default=1.0)
...
def build_model(args, RS, device):
    from models.GSTMSCD_MTSCD_Snn_ForDecoder_clean_evidence_unit import GSTMSCD_WUSU as Net
    ...
    model = Net(
        ...
        detach_pdca_guidance=args.detach_pdca_guidance,
        use_pdca_guidance=args.use_pdca_guidance,
        collect_pdca_aux=args.collect_pdca_aux,
        task_evidence_cfg=dict(
            enabled=args.task_evidence_enabled,
            feature_residual=args.task_evidence_feature_residual,
            use_highpass_evidence=args.task_evidence_use_highpass,
            detach_evidence=args.task_evidence_detach,
            normalize_evidence=args.task_evidence_normalize,
            use_integer_surrogate=args.task_evidence_use_integer_surrogate,
            residual_init=args.task_evidence_residual_init,
            scale=args.task_evidence_scale,
        ),
        decoder_task_gate_cfg=dict(
            enabled=args.decoder_task_gate_enabled,
            use_pdca_source_weights=args.decoder_task_gate_use_pdca_source_weights,
            detach_evidence=args.decoder_task_gate_detach_evidence,
            detach_pdca_guidance=args.decoder_task_gate_detach_pdca_guidance,
            pdca_scale_key=args.decoder_task_gate_pdca_scale_key,
            alpha_max=args.decoder_task_gate_alpha_max,
        ),
