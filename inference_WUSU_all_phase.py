...
from models.GSTMSCD_MTSCD import GSTMSCD_WUSU_Random as Net
...
# from models.FEMCD.FEMCD import FEMCD_net_dynamic as Net
from utils.palette import color_map_DynamicEarth, color_map_WUSU12, color_map_WUSU13
...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--data_root", type=str,
                            default=r"/home/think/ljh_datasets/WUSU/val")
        # /media/think/data/ljh/GSTM_main/checkpoints/WUSU/GSTMSCD/GOST-Mamba/epoch88_Score67.40_mIOU89.12_Sek58.09_Fscd86.97_OA98.28.pth
        #  /media/think/data/ljh/GSTM_main/logs/clean_kmode_NoQlif_bs1/Best Model: epoch97_Score55.47_mIOU82.39_Sek43.93_Fscd79.43_OA96.16.pth
        # /media/think/data/ljh/GSTM_main/logs/WUSU/GSTMSCD/GOST-Mamba/epoch93_Score57.24_mIOU83.26_Sek46.09_Fscd80.44_OA96.33.pth
        parser.add_argument("--load_from", type=str,
                            default=r"/media/think/data/ljh/GSTM_main/checkpoints/WUSU/GSTMSCD/GOST-Mamba/epoch88_Score67.40_mIOU89.12_Sek58.09_Fscd86.97_OA98.28.pth")
...
def inference_ss(args, pair):
    ...
    # model = Net(4, num_classes=len(RS.ST_CLASSES)-1)
    if args.load_from:
    ...
    if args.test_batch_size > 0:
        for vi, data in enumerate(testloader):
            if vi == 0:
                img1, img2, img3, label1, label2, label3, label_bn, id = data
                img1, img2, img3 = img1.cuda().float(), img2.cuda().float(), img3.cuda().float()
                FLOPs, Params = profile(model, (img1, img2, img3, pair))
                # FLOPs, Params = profile(model, (img1, img2, img3))
    ...
    with torch.no_grad():
        for img1, img2, img3, label1, label2, label3, label_bn, id in tbar:
            ...
            out1, out2, out3, out_bn = model(img1, img2, img3, pair)
            # out1, out2, out3, out_bn = model(img1, img2, img3)
            # 直接获取预测结果，避免不必要的张量复制
            # outputs = [torch.argmax(out, dim=1).cpu().numpy() + 1 for out in [out1, out2, out3]]
            outputs = [torch.argmax(out, dim=1).cpu().numpy() for out in [out1, out2, out3]]
            # 保存所有时相的语义分割结果
...
def inference_scd(args, pair):
    ...
    with torch.no_grad():
        for img1, img2, img3, label1, label2, label3, label_bn, id in tbar:
            ...
            # out1, out2, out3, out_bn = model(img1, img2, img3)
            # 直接获取预测结果，避免不必要的张量复制
            # seg_predictions = [torch.argmax(out, dim=1).cpu().numpy() + 1 for out in [out1, out2, out3]]
            seg_predictions = [torch.argmax(out, dim=1).cpu().numpy() for out in [out1, out2, out3]]
